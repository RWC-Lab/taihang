...
struct BSGSConfig {
    ...
    bool   use_batch_norm = true;     // Batch affine normalization switch
    size_t batch_norm_size    = 32;        // Points per batch norm (32 optimal for P-256)
};
...
public:
    ...
    //   - Robin Hood collision resolution (minimizes variance in probe length)
...
private:
    ...
    // Standard path: one field inversion per point via xxhash_to_uint64
    void build_sliced_table(ECPoint start_point, size_t start_index, size_t slice_size, uint8_t* buffer);

    // Batched path: one multi-inversion per batch_size points
    void build_sliced_table_batched(ECPoint start_point, size_t start_index, size_t slice_size, uint8_t* buffer);

    // Standard giant step search: amortizes inversion cost across batch_size steps
    void search_sliced_giantstep(ECPoint target_point, size_t start_giant_index, 
                                 size_t slice_size,  BigInt& result, std::atomic<bool>& found) const;

    // Batched giant step search: amortizes inversion cost across batch_size steps
    void search_sliced_giantstep_batched(ECPoint target_point, size_t start_giant_index, 
                                         size_t slice_size, BigInt& result, std::atomic<bool>& found) const;
};
