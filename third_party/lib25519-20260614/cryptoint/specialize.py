...
for bits in 8,16,32,64:
  for ui in 'int','uint':
    ...
    with open(f'{TYPE}.h','w') as f:
      ...
      gnuasm = '(defined(__GNUASM__) || (defined(__GNUC__) && !defined(__FILC__)))'
      contents = contents.replace('amd64',f'{gnuasm} && defined(__x86_64__)')
      contents = contents.replace('arm64',f'{gnuasm} && defined(__aarch64__)')
      contents = contents.replace('arm32',f'{gnuasm} && defined(__arm__) && defined(__ARM_ARCH) && (__ARM_ARCH >= 6) && !defined(__thumb__)')
      contents = contents.replace('sparc32',f'{gnuasm} && defined(__sparc_v8__)')
