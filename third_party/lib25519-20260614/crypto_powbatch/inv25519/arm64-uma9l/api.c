...
static const fe25519_9l_dense one = {{0,0,0x0000000100000000,0,0}};

void crypto_powbatch(unsigned char *q,const unsigned char *p,long long batch)
{
  fe25519_9l_dense tmp[MAXTODO+MAXTODO-1];
  /* for todo=1: p0 */
  ...
  while (batch > 0) {
    ...
    for (long long pos = 0;pos < todo;++pos) {
      fe25519_unpack(&x,p);
      fe25519_4l_to_9l_dense(&tmp[todo-1+pos],&x);
      p += 32;
      fe25519_9l_dense_to_4l(&x,&tmp[todo-1+pos]);
      fe25519_pack(s,&x);
    ...
    fe25519_9l_dense_to_4l(&x,&tmp[0]);
    fe25519_pack(s,&x);
    crypto_pow_inv25519(s,s);
    fe25519_unpack(&x,s);
    fe25519_4l_to_9l_dense(&tmp[0],&x);    
