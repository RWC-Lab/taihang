 ...
 *            across thread counts and tradeoff parameters.
 * @author    Yu Chen
 *****************************************************************************/
...
int main() {
    ...
    // --------------------------------------------------------
    // 2. Tradeoff Parameter Sweep
    //    Fixed: threads=DEFAULT
    //
    //    tradeoff=k:
    //      baby  steps = 2^(range/2 + k)  → more memory, larger build
    //      giant steps = 2^(range/2 - k)  → fewer iterations, faster solve
    //    tradeoff=0: balanced (baby == giant == 2^(range/2))
    //    tradeoff=DEFAULT: memory-heavy, fastest solve
    // --------------------------------------------------------
    print_header("2. Tradeoff Parameter Sweep"
    ...
    // --------------------------------------------------------
    // 3. Range Scaling
    //    Fixed: threads=DEFAULT, tradeoff clamped to min(DEFAULT, range/2-1)
    // --------------------------------------------------------
    print_header("3. Range Scaling"
                 "  (threads=" + std::to_string(DEFAULT_THREADS) +
    ...
    // --------------------------------------------------------
    // 4. Summary: Baseline vs Best Config
    //    Baseline: 1 thread, tradeoff=0  (no parallelism, balanced steps)
    //    Best:     DEFAULT threads, tradeoff=DEFAULT  (full parallelism, memory-heavy)
    // --------------------------------------------------------
    print_header("4. Summary: Baseline vs Best Config");
    ...
    }
    {
        auto r = run_bench(group, g,
            {.range_bits   = RANGE_BITS,
             ...
             .thread_num   = DEFAULT_THREADS},
            SOLVE_ITERS);
        print_row("Best (t=" + std::to_string(DEFAULT_THREADS) +
